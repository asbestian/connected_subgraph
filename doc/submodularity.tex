...
\begin{frame}{Steiner tree problem}
  ...
  \begin{tikzpicture}[node distance = 2cm]
    \node[draw,circle](A) {$a$};
    \node[draw,circle, red](B) [right of=A] {$b$};
    \node[draw,circle, red](C) [below of=A] {$c$};
    \node[draw,circle](D) [right of=C] {$d$};
    \node[draw,circle, red](E) [right of=D] {$e$};
...
\begin{frame}{Proof}
  \begin{proof}
    \begin{align*}
      f(S \cup \{i\}) - f(S) =
      %%-\sum\limits_{T \subseteq S \cup \{j\}} c_T + \sum\limits_{T \subseteq S} c_T + &\sum\limits_{T \cap (S \cup \{j\}) \neq \emptyset} r_T - \sum\limits_{T \cap S \neq \emptyset} r_T = \\
...
\begin{frame}{Depiction of max-flow problem}
  \begin{center}
    \begin{tikzpicture}[node distance = 1.5cm]
      \node[draw,circle](s) {$s$};
      \node[draw,circle](B) [right of=s] {};
      \node[draw,circle](A) [above of=B] {};
      \node[draw,circle](C) [below of=B] {$I$};
      \node[] [above of=A] {$V_1$};
      \node[draw,circle](E) [right of=B] {$J$};
      \node[draw,circle](D) [above of=E] {};
      \node[draw,circle](F) [below of=E] {};
      \node[] [above of=D] {$V_2$};
      \node[draw,circle](t) [right of=E] {$t$};
      \draw[->] (s) -- (A);
      \draw[->] (s) -- (B);
      \draw[->] (s) -- (C) node[midway,below] {$c_I$};
      \draw[->] (C) -- (E) node[midway] {\small{$I \cap J \neq \emptyset$}};
      \draw[->] (E) -- (t) node[midway,below] {$r_J$};
    \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}{Separation revisited}
  \begin{itemize}
  \item we want to solve $\min\limits_{\substack{S \subseteq V,\\S
      \cap T \neq \emptyset}} f(S)$
  \item[]
  \item define $V_j = V \setminus \{j\}$ and $f_j(S) = f(S \cup
    \{j\})$ for $j \in T$
  \item[]
  \item $f_j(S) = \sum\limits_{i \in N \cap (S \cup \{j\})} y^*_i +
    \sum\limits_{i \in T \cap (S \cup \{j\})} 1 - \sum\limits_{e \in
      E(S \cup \{j\})} x^*_e$
  \item[]
  \item solve $\min\limits_{j \in T} \min\limits_{S \subseteq V_j}
    f_j(S)$
  \end{itemize}
\end{frame}

\section{Example}
\begin{frame}[fragile]{Example instance}
...
\begin{frame}{Optimal solution of LP relaxation}
  \begin{itemize}
  \item $\sum\limits_{e \in E(S)} x_e \leq \sum\limits_{i \in N \cap S} y_i + |T \cap S| - 1~~\forall S \subseteq V,~S \cap T \neq \emptyset$
  \end{itemize}
  \vspace{1cm}
  ...
  \begin{center}
    \begin{tikzpicture}[node distance = 2cm]
    \node[draw,circle, blue](1) {$1$};
    \node[draw,circle, red](2) [right of=1] {$2$};
    \node[draw,circle, red](3) [below of=1] {$3$};
    \node[draw,circle](4) [right of=3] {$4$};
    \node[draw,circle, blue](5) [right of=4] {$5$};
    \draw (1) -- (3);% node[midway,left] %{$e_{13}$};
    \draw (1) -- (4);% node[midway,above] {$e_{14}$};
  ...
  \end{minipage}
  \begin{itemize}
  \item $\Rightarrow x_{24} + x_{25} + x_{45} \leq y_2 + y_4$
  \end{itemize}
\end{frame}
